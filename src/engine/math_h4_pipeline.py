"""Math portfolio pipeline on H4 (vs the live M15 portfolio).
...
Outputs:
  reports/Math_H4_Phase_A.md / .parquet
  reports/Math_H4_Phase_B.md / .parquet
  reports/Math_H4_Optuna.md / .parquet
  reports/Math_H4_Robustness.md / .parquet
"""
...
H1_CACHE = Path("data/enriched_math_h4")
TF = "H4"
...
def phase_a():
    ...
    df.write_parquet(REPORTS_DIR / "math_h4_phase_a.parquet")
...
def phase_b(phase_a_df: pl.DataFrame) -> pl.DataFrame:
    ...
    df.write_parquet(REPORTS_DIR / "math_h4_phase_b.parquet")
...
def phase_c_optuna(phase_b_df: pl.DataFrame) -> pl.DataFrame:
    ...
    df.write_parquet(REPORTS_DIR / "math_h4_optuna.parquet")
...
def phase_d_robustness(opt_df: pl.DataFrame) -> pl.DataFrame:
    ...
    if not rows:
        return pl.DataFrame()
    df = pl.DataFrame(rows)
    df.write_parquet(REPORTS_DIR / "math_h4_robustness.parquet")
    ...
    md_lines = ["# Math H4 Pipeline — Robustness", "",
    ...
    (REPORTS_DIR / "Math_H4_Robustness.md").write_text("\n".join(md_lines) + "\n", encoding="utf-8")
    return df
...
def main():
    ...
    print("\nDone. Reports:")
    print("  reports/math_h4_phase_a.parquet")
    print("  reports/math_h4_phase_b.parquet")
    print("  reports/math_h4_optuna.parquet")
    print("  reports/Math_H4_Robustness.md / .parquet")
