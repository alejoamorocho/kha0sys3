...
# Note: _load_and_enrich_math (formerly src.engine.run_math_discovery) was
# removed from runtime — only the discovery CLI functions inside this module
# need it, so we import it lazily there. The K3M1-75 runtime (live_math_trader
# + math_order_manager) only imports detect_setups and _guard_indicator_col
# from this module, which do NOT touch the discovery helpers.
...
# Phase A/B/C discovery CLI removed in K3M1-75 cleanup.
# Use src/engine/k3_universe_m1_mgmt.py for the current discovery pipeline.
