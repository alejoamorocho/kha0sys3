"""ORB break-fade Phase J2 — same grid as Phase J but with realistic Vantage
Markets friction (0.05R FX / 0.10R commodities+indices).

Diagnosed root cause of Phase J zero survivors: friction was 0.3R/0.4R which
is 2-4× overstated vs Vantage Pro/ECN real costs.

Grid (identical to Phase J):
...
Friction override (Vantage realistic):
  FX (EURUSD, USDJPY, GBPAUD): 0.05R  (was 0.3R in Phase J)
  Commodities/Indices (all others): 0.10R  (was 0.4R in Phase J)
...
Usage:
    python -u -m src.strategies_external.runners.run_orb_breakfade_phase_j2
"""
...
# ── Phase J2 grid (identical to Phase J) ─────────────────────────────────────
SYMS = [
...
# ── Friction override: Vantage realistic (was 0.3/0.4 in Phase J) ────────────
_FX_SET = frozenset({"EURUSD", "USDJPY", "GBPAUD"})


def _friction_for(sym: str) -> float:
    """Realistic Vantage Markets friction in R-multiples.

    FX majors (tight spread + low commission): 0.05R
    Metals, energy, indices (slightly wider): 0.10R

    Previous Phase J values (0.3R / 0.4R) were 2-4x overstated.
    """
    if sym in _FX_SET:
        return 0.05
    return 0.10
...
def run_phase_j2(
    output_stem: str = "reports/external/orb_breakfade_phase_j2",
) -> pl.DataFrame:
    ...
    print(
        f"[PhaseJ2] Grid: {len(SYMS)} syms x {len(MAGIC_TIMES)} magic_times x "
        f"{len(DURATIONS)} durations x {n_rr} TP/SL = {total_expected} backtests",
        flush=True,
    )
    print(
        f"[PhaseJ2] ATR window: {ATR_WINDOW}  |  "
        f"Strict gates: WR>={MIN_WR} AND PF>={MIN_PF} AND n>={MIN_TRADES}",
        flush=True,
    )
    print(
        "[PhaseJ2] Friction override: FX=0.05R (was 0.3), "
        "Commodities/Indices=0.10R (was 0.4)",
        flush=True,
    )
    ...
    for sym in SYMS:
        try:
            ...
            print(f"[PhaseJ2][SKIP] {sym} — no M15 data", flush=True)
            m15_cache[sym] = None
        m1_df = load_m1(sym)
        m1_cache[sym] = m1_df
        if m1_df is None:
            print(f"[PhaseJ2][SKIP] {sym} — no M1 data", flush=True)
        elif m15_cache[sym] is not None:
            print(
                f"[PhaseJ2][Load] {sym}: M1 {len(m1_df):,} rows, "
                f"M15 {len(m15_cache[sym]):,} rows",
                flush=True,
            )

    # ── Phase 1: Signal generation cache ─────────────────────────────────────
    print("\n[PhaseJ2] === Phase 1: Signal generation cache ===", flush=True)
    ...
    print("\n[PhaseJ2] === Phase 2: TP/SL grid sweep ===", flush=True)
    ...
    for sym in SYMS:
        ...
        friction_r = _friction_for(sym)   # <-- overridden: 0.05 or 0.10
                ...
                for tp in TP_GRID:
                    for sl in SL_GRID:
                        ...
                        if backtest_count % 500 == 0:
                            ...
                            print(
                                f"[PhaseJ2] {backtest_count}/{total_expected} "
    ...
    print(
        f"\n[PhaseJ2] complete in {elapsed:.0f}s ({elapsed / 60:.1f} min)"
    ...
    if not df_survivors.is_empty():
        ...
        print(f"[PhaseJ2] Survivors parquet: {output_stem}.parquet", flush=True)
    if not df_all.is_empty():
        df_all.write_parquet(f"{output_stem}_all.parquet")
        print(f"[PhaseJ2] All-results parquet: {output_stem}_all.parquet", flush=True)
...
def _write_report(
    ...
    out_md.write_text("\n".join(lines) + "\n", encoding="utf-8")
    print(f"[PhaseJ2] Report written to {out_md}", flush=True)


if __name__ == "__main__":
    # ── Sanity test: compare GBPAUD 15:00 120m TP=0.7 SL=0.5 with old vs new friction ──
    # Phase J top closest-to-gate: WR=0.486, PF=1.123 with friction=0.30
    # With friction=0.05 (J2) we expect significantly higher PF
    print(
        "[PhaseJ2] Sanity test: GBPAUD 15:00 120m TP=0.7 SL=0.5 atr_w=14",
        flush=True,
    )
    print(
        "  Expected: same WR as Phase J but higher PF with 0.05 vs 0.30 friction",
        flush=True,
    )
    from src.strategies_external.constants import friction_for as _old_friction_for
    from src.strategies_external.runners.run_orb_breakfade_phase_h import (
        _precompute_m1_arrays as _pma,
        _run_orb_with_alt_exit as _roae,
    )

    _adapter_st = ORBBreakFadeAdapter()
    try:
        _df_m15_st = load_csv("GBPAUD", "M15")
    except FileNotFoundError:
        _df_m15_st = None
    _m1_df_st = load_m1("GBPAUD")

    _sanity_ok = False
    if _m1_df_st is not None and _df_m15_st is not None:
        _m1_arrays_st = _pma(_m1_df_st)
        _base_st = _adapter_st.generate_signals_for_combo(
            _df_m15_st, _m1_df_st, "GBPAUD", "15:00", 120, atr_window=14
        )
        print(f"  Signals generated: {len(_base_st)}", flush=True)
        if _base_st:
            _sigs_st = _attach_rr(_base_st, tp_mult=0.7, sl_mult=0.5)

            # Old friction (Phase J)
            _old_fr = _old_friction_for("GBPAUD")
            _trades_old = _roae(_sigs_st, _m1_arrays_st, exit_mode="baseline", friction_r=_old_fr)
            _m_old = evaluate(_trades_old)
            print(
                f"  Phase J  (friction={_old_fr:.2f}R): n={_m_old['n_trades']} "
                f"WR={_m_old['win_rate']:.3f} PF={_m_old['profit_factor']:.3f}",
                flush=True,
            )

            # New friction (Phase J2)
            _new_fr = _friction_for("GBPAUD")
            _trades_new = _roae(_sigs_st, _m1_arrays_st, exit_mode="baseline", friction_r=_new_fr)
            _m_new = evaluate(_trades_new)
            print(
                f"  Phase J2 (friction={_new_fr:.2f}R): n={_m_new['n_trades']} "
                f"WR={_m_new['win_rate']:.3f} PF={_m_new['profit_factor']:.3f}",
                flush=True,
            )

            _pf_improved = _m_new["profit_factor"] > _m_old["profit_factor"]
            _sanity_ok = _m_new["n_trades"] >= 1 and 0.0 <= _m_new["win_rate"] <= 1.0
            print(
                f"  Sanity: {'PASS' if _sanity_ok else 'FAIL'}  "
                f"PF improved: {'YES' if _pf_improved else 'NO'}",
                flush=True,
            )
        else:
            print(
                "  [WARN] 0 signals for GBPAUD 15:00 120m — sanity inconclusive",
                flush=True,
            )
    else:
        print(
            "  [WARN] Missing GBPAUD M1 or M15 data — sanity test skipped",
            flush=True,
        )

    print("\n[PhaseJ2] Starting full sweep...\n", flush=True)
    _df_result = run_phase_j2()
    print(f"\n[PhaseJ2] Done. Survivors: {len(_df_result)}", flush=True)
