# src/strategies_external/runners/run_perdices_fib.py
"""Runner Perdices Fib: backtest con doc + indicator + ATR sweep.

Uses H1 bars directly as signal TF (no aggregate_to_daily call).
Symbols: XAUUSD, XAGUSD.
ATR grid limited to 5 combos for reasonable run time.
"""
...
def run_perdices_fib_backtest(
    symbols: list[str],
    data_dir: str = "data",
    output_path: Path | str = "reports/external/perdices_fib_backtest.md",
    atr_grid: list[tuple[float, float, float]] | None = None,
) -> dict[str, dict]:
    # Default: 5 combos to keep run time reasonable (full 36-combo sweep is too slow)
    if atr_grid is None:
        atr_grid = [
            (1.0, 1.5, 3.0),
            (1.5, 2.0, 4.0),
    ...
    _trades_to_parquet(all_trades, output_path.parent / "perdices_fib_trades.parquet")
...
if __name__ == "__main__":
    ...
    print("Perdices Fib backtest done.")
