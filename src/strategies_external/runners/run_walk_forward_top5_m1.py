"""Walk-Forward IS/OOS Validation — Top 5 M1 Phase K2/K-F Candidates.
...
Candidates are the top-5 high-frequency M1 entries from Phase K2/K-F
(n=1500-1900 trades, ~210 IS + ~90 OOS per window).

Usage:
    python -u -m src.strategies_external.runners.run_walk_forward_top5_m1
"""
...
OUTPUT_MD = REPORTS_DIR / "math_discovery_m1_phase_k2_walkforward_m1.md"
...
_FX_SET = {"EURUSD", "USDJPY", "GBPAUD", "GBPUSD", "AUDUSD", "EURJPY",
           "GBPJPY", "USDCAD", "USDCHF", "NZDUSD"}

CANDIDATES: list[dict] = [
    {
        "label": "EURUSD M1 HURST_TREND_MOM ASIA INV=False TP=1.0 SL=1.0",
        "sym": "EURUSD", "tf": "M1", "setup": "HURST_TREND_MOM",
        "ses": "ASIA", "inv": False, "is_fade": False,
        "tp": 1.0, "sl": 1.0,
    },
    {
        "label": "USDJPY M1 VELOCITY_ACCEL_GO ASIA INV=False TP=1.0 SL=1.0",
        "sym": "USDJPY", "tf": "M1", "setup": "VELOCITY_ACCEL_GO",
        "ses": "ASIA", "inv": False, "is_fade": False,
        "tp": 1.0, "sl": 1.0,
    },
    {
        "label": "EURUSD M1 KALMAN_INNOV_EXPAND ALL_DAY INV=True TP=1.0 SL=1.0",
        "sym": "EURUSD", "tf": "M1", "setup": "KALMAN_INNOV_EXPAND",
        "ses": "ALL_DAY", "inv": True, "is_fade": False,
        "tp": 1.0, "sl": 1.0,
    },
    {
        "label": "EURUSD M1 VELOCITY_ACCEL_GO ALL_DAY INV=True TP=1.0 SL=1.0",
        "sym": "EURUSD", "tf": "M1", "setup": "VELOCITY_ACCEL_GO",
        "ses": "ALL_DAY", "inv": True, "is_fade": False,
        "tp": 1.0, "sl": 1.0,
    },
    {
        "label": "USDJPY M1 VELOCITY_ACCEL_GO ALL_DAY INV=False TP=1.0 SL=1.0",
        "sym": "USDJPY", "tf": "M1", "setup": "VELOCITY_ACCEL_GO",
        "ses": "ALL_DAY", "inv": False, "is_fade": False,
        "tp": 1.0, "sl": 1.0,
    },
]
...
def run_candidate(cand: dict) -> dict:
    ...
    # Load enriched bars — for M1 candidates the signal TF is M1
...
def _write_report(results: list[dict], runtime_s: float) -> None:
    ...
    lines = [
        ...
        f"- No re-optimization in IS — fixed TP=1.0 SL=1.0 from K-F phase",
        f"- Friction: 0.05R FX / 0.10R non-FX",
        f"- Expected per window: ~210 IS trades + ~90 OOS trades (n=1500-1900 total)",
...
def run() -> None:
    t_start = time.time()
    print("=" * 70, flush=True)
    print("[WF] Walk-Forward IS/OOS — Top 5 M1 Phase K2/K-F Candidates", flush=True)
    ...
    for i, res in enumerate(results, 1):
        if res.get("error"):
            print(f"  C{i}: {res['label'][:60]}... SKIPPED ({res['error']})", flush=True)
            continue
        n_valid += 1
